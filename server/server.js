...
function readIdentityCookie(req) {
  ...
  for (const part of raw.split(';')) {
    ...
    if (part.slice(0, i).trim() === ID_COOKIE) {
      ...
      // AP1-01 (#220): a malformed percent-escape ("%zz") made
      // decodeURIComponent throw URIError. The middleware calls this on EVERY
      // request, so one broken cookie turned the whole site into 500s. A cookie
      // we cannot decode is simply not an identity.
...
const DOMAIN_MAX_LEN = 120;   // the column width, stricter than RFC 1035's 253
