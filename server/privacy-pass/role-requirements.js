...
export const ROLE_REQUIREMENTS = {
  ...
  student: {
    ...
    emailDomainHint:    'University/college email (.uni-*, .tu-*, .hs-*, .ac.de, .edu)',
  },
  teacher: {
    required: ['webauthn', 'email-verified'],
    optional: ['teacher-id'],
    emailDomainPattern: /(^|\.)(schule\.[\w-]+\.de|bbs\.[\w-]+\.de|bildung\.[\w-]+\.de|schule\.[\w-]+|gymnasium\.[\w-]+\.de)$|\.edu$/i,
    emailDomainHint:    'School email (.schule.[state].de, .bbs.*, .bildung.*)',
  },
  researcher: {
    required: ['webauthn', 'email-verified'],
    optional: ['orcid'],
    ...
    emailDomainHint:    'University/institution email (.uni-*, .tu-*, .mpg.de, .fraunhofer.de, .dlr.de, .helmholtz.de, .leibniz.de, .edu)',
  },
  creative: {
    required: ['webauthn', 'email-verified'],
  ...
  medical_professional: {
    ...
    emailDomainHint:    'Clinic, practice, or university-hospital domain',
    strict: true,
    strictReason: 'Medical professional identity requires confirmation of a medical licence (Approbation). Without a licence number, no tokens can be issued for this role.',
  },
  caregiver: {
    required: ['webauthn', 'email-verified'],
    optional: ['care-chamber-id'],
    emailDomainPattern: /(klinik[\w-]*\.|pflege[\w-]*\.|caritas|diakonie|drk|asb-|johanniter|malteser)/i,
    emailDomainHint:    'Clinic, care, or welfare-organisation email',
  },
  lawyer: {
    required: ['webauthn', 'email-verified', 'bar-association-id'],
    optional: [],
    emailDomainPattern: /(rechtsanwalt|kanzlei|anwalt|brak\.de|notar)/i,
    emailDomainHint:    'Law-firm or bar-association email',
    strict: true,
    strictReason: 'Attorney identity requires an entry in the bar association register.',
  },
  notary: {
    required: ['webauthn', 'email-verified', 'notary-chamber-id'],
    optional: [],
    strict: true,
    strictReason: 'Notary identity requires an entry in the notary chamber — the public office demands the highest trust level.',
  },
  civil_servant: {
    required: ['webauthn', 'email-verified'],
    optional: ['service-id'],
    ...
    emailDomainHint:    'Government email (.bund.de, [state].de, polizei.*, finanzamt.*)',
    strict: true,
    strictReason: 'Public-authority identity requires an official government email address.',
  },
  politician: {
    required: ['webauthn', 'email-verified'],
    optional: ['bundestag-verified'],
    emailDomainPattern: /(^|\.)(bundestag\.de|bundesrat\.de|bundesregierung\.de|landtag\.[\w-]+\.de|abgeordnete[\w.-]*)$/i,
    emailDomainHint:    'Official representative email (@bundestag.de, @landtag.*, @bundesregierung.de)',
    strict: true,
    strictReason: 'Elected-representative identity requires an official parliamentary or government email.',
  },
  business: {
    required: ['webauthn', 'email-verified'],
...
export function checkEligibility(role, completedMethods) {
  ...
  if (missing.length > 0) {
    return {
      ...
      reason: req.strictReason || `Missing: ${missing.join(', ')}`,
    };
  }
